using System;
using System.IO;
using System.Text.Json;
using ArknightsLauncher.Models;

namespace ArknightsLauncher.Services
{
    public static class ConfigService
    {
        public static readonly string ConfigDir =Path.Combine(Environment.GetEnvironmentVariable("LOCALAPPDATA")!,
            "ArknightsLauncher");

        public static readonly string ConfigFile =
            Path.Combine(ConfigDir, "config.json");

        public static readonly string AccountBackupDir =
            Path.Combine(ConfigDir, "AccountBackups");

        public static AppConfig Load()
        {
            if (!File.Exists(ConfigFile)) return new AppConfig();
            try
            {
                return JsonSerializer.Deserialize<AppConfig>(File.ReadAllText(ConfigFile))
                       ?? new AppConfig();
            }
            catch
            {
                return new AppConfig();
            }
        }

        public static void Save(AppConfig cfg)
        {
            Directory.CreateDirectory(ConfigDir);
            File.WriteAllText(ConfigFile,
                JsonSerializer.Serialize(cfg, new JsonSerializerOptions { WriteIndented = true }));
        }
    }
}
